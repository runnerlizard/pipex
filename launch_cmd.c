...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/18 18:09:40 by cluco             #+#    #+#             */
/*   Updated: 2022/01/18 18:16:00 by cluco            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (p->file1 < 0)
		close_and_free("61230", p, argv[1], 0);
	if (dup2(p->file1, STDIN_FILENO) < 0)
		close_and_free("561230", p, "dup2", 0);
	...
	if (p->pid2 == 0)
		...
		close_and_free("4561230", p, "fork1", 0);
}
...
{
	if (wait(NULL) < 0)
		close_and_free("4561230", p, "wait", 0);
	p->file2 = open(argv[4], O_TRUNC | O_CREAT | O_WRONLY, 0664);
	if (p->file2 < 0)
		close_and_free("4561230", p, argv[4], 0);
	p->tmp_fd = open("clucotmpfile", O_TRUNC | O_CREAT | O_RDWR, 0664);
	if (p->tmp_fd < 0)
		close_and_free("561230", p, "clucotmpfile", 0);
	p->pid2 = fork();
	if (p->pid2 == 0)
		...
		close_and_free("4561230", p, "fork2", 0);
}
